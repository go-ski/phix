...
# Bake EXIF orientation into the pixels (emulates exiftool/-auto-orient) so
# the re-encoded thumbnail always shows upright in the browser.
# Not exported — used internally by make_thumb().
...
# Not exported — called from app_server().
make_thumb <- function(path, orient, max_px = 1000) {
  ...
  out <- file.path(.phix_env$THUMB_DIR,
